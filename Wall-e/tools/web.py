...
def search_web(query: str, source: str = "web") -> str:
    ...
    Use this when the answer is not on this machine. source="web" searches
    the wider web (official Google API when GOOGLE_API_KEY and GOOGLE_CSE_ID
    are set in config.py, DuckDuckGo fallback without keys).
    source="wikipedia" looks up encyclopedic topics (people, places,
    history, science) via the official Wikipedia API.

    Args:
        query: What to search for, for example "Alan Turing" or "KDE 6 release notes".
        source: "web" for general search, "wikipedia" for encyclopedic topics.
    """
    if source == "wikipedia":
...
# Legacy names kept importable (not advertised as tools) so old code that
# does `from tools.web import search_google` keeps working after the rename.
